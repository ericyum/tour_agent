...
def display_paginated_gallery(results, page_str, direction):
    ...
    if 1 <= new_page <= total_pages:
        gallery_output, page_display_str, *page_buttons_updates = display_page(
            results, new_page
        )
...
def run_search_and_display(area, sigungu, main_cat, medium_cat, small_cat, status):
    ...
    gallery, page_str_updated, *page_buttons_updates = display_page(
        results_for_state, 1
    )
    return (
        results_for_state,
...
async def display_festival_details_and_precautions(
    ...
    ), gr.update(visible=True), gr.update(visible=True), gr.update(
        visible=True
    ), gr.update(
        visible=True
    ), gr.update(
...
async def handle_rank_festivals(
    ...
    print(f"DEBUG: Calculated total_pages in handle_rank_festivals: {total_pages}")

    report_update = gr.update(value=report_md, visible=True)
    print(
        f"DEBUG: Exiting handle_rank_festivals. Ranked festivals length: {len(ranked_festivals)}, Page display: {page_display_str}, Total pages: {total_pages}"
    )
    return (
        ranked_festivals,
        gallery_output,
        ...
        *page_buttons_updates,
        report_update,
        total_pages,  # Added total_pages here
    )
...
async def handle_rec_last_page(results):
    print(
        f"DEBUG: handle_rec_last_page called with results length: {len(results) if results else 0}"
    )
    if not results:
        return (
            gr.update(value=[]),
...
def handle_rec_last_page_click(results, total_pages):
    print(
        f"DEBUG: Entering handle_rec_last_page_click. Results length: {len(results) if results else 0}, Total pages: {total_pages}"
    )
    return display_page(results, total_pages)
