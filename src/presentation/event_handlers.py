...
from src.infrastructure.config.loader import (
    ICON_MAP, KOREAN_FONT_PATH, TITLE_TO_CAT_NAMES,
    ALL_FESTIVAL_CATEGORIES, FESTIVAL_INFO_LOOKUP, BEST_IMAGES_MAP,
)
from src.application.core.constants import (
    CATEGORY_TO_ICON_MAP, NO_IMAGE_URL, PAGE_SIZE,
    COLUMN_TRANSLATIONS, AREA_CODE_MAP, SIGUNGU_CODE_MAP,
)
...
from src.infrastructure.reporting.charts import (
    create_donut_chart, create_sentence_score_bar_chart,
)
...
# [수정] 이 함수는 더 이상 필요하지 않습니다. (ui.py에서 탭으로 대체)
# def switch_page(page_name: str):
#     ...

# --- Asset Path Functions (기존과 동일) ---
...
def display_page(results, page):
    ...
    return gallery_output, f"{page} / {total_pages}", *page_buttons_updates, checkbox_update
...
def display_paginated_gallery(results, page_str, direction):
    ...
    if 1 <= new_page <= total_pages:
        ...
        gallery_output, page_display_str, *page_buttons_updates, checkbox_update = display_page(
            results, new_page
        )
        ...
        # [수정] checkbox_update 반환값 추가 (총 8개)
        gallery_output, page_display_str, *page_buttons_updates, checkbox_update = display_page(
            results, current_page_to_display
        )
        # [수정] 반환값에 체크박스 업데이트 추가
        return gallery_output, page_display_str, *page_buttons_updates, checkbox_update
...
async def display_festival_details_and_precautions(
    ...
    score_keys = {
        ...
        "sentiment_score": "만족도 점수", "quarterly_trend_score": "최근 화제성(90일)",
...
async def handle_gallery_select_and_show_details(
    evt: gr.SelectData, results, page_str
):
    """ [*** 수정됨 ***]
    ...
    details_outputs = {}
    async for output in display_festival_details_and_precautions(evt, results, page_str):
        details_outputs = {
            "details_text": output[0],
            "original_title": output[1],
            "details_dict": output[2],
            "precautions_text": output[3],
            "selected_item": output[4],
        }
...
async def get_naver_review_info(festival_name, num_reviews):
    ...
    if not festival_name:
        ...
        return
    yield gr.update(value=f"{festival_name} 후기 검색 중... ({num_reviews}개)", visible=True)
...
async def handle_generate_word_cloud(festival_name, num_reviews):
    """ (기존 핸들러 - 출력값만 수정) """
    yield gr.update(value=f"워드 클라우드 생성 중... ({num_reviews}개)", visible=True), None
...
async def handle_analyze_sentiment(festival_name, num_reviews):
    ...
    try:
        ...
        yield (
            ...
            *[item for s in ["봄", "여름", "가을", "겨울"] for item in (
                gr.update(value=seasonal_pos_wc.get(s), visible=s in seasonal_pos_wc),
                gr.update(value=seasonal_neg_wc.get(s), visible=s in seasonal_neg_wc),
            )],
            initial_page_df,
            result["blog_df"],
            ...
            gr.update(visible=False), gr.update(visible=False, open=False),
        )
        return
    except Exception as e:
...
async def handle_rank_festivals(
    ...
    if not festivals_list:
        ...
        return (
            [], gr.update(value=[]), "1 / 1",
            *([gr.update(visible=False)] * 5),
            gr.update(value="", visible=False), 1,
    ...
    return (
        ranked_festivals, gallery_output, page_display_str,
        *page_buttons_updates, report_update, total_pages,
        checkbox_update # 1페이지에 맞는 체크박스
    )
...
async def handle_rank_selected_festivals(
    ...
    if not selected_titles:
        empty_updates = (
            [], gr.update(value=[]), "1 / 1",
            *([gr.update(visible=False)] * 5),
            gr.update(value="순위를 매길 축제를 선택해주세요.", visible=True), 1,
            gr.update(choices=[], value=[]) # 빈 체크박스
        )
...
def run_nearby_search(festival_details, radius_meters):
    ...
    return (
        facilities_recs, courses_recs, festivals_recs,
        gr.update(visible=False),
        facility_gallery, course_gallery, festival_gallery,
...
def handle_item_selection(evt: gr.SelectData, items: list):
    ...
    exclude_cols = [
        ...
        "modifiedtime", "sigungucode", "ranking_score", "time_score",
        "sentiment_score", "quarterly_trend_score", "yearly_trend_score",
        "sub_points", "trend_reason", "sentiment_reason",
    ]
...
def add_to_my_course(item_to_add, current_course: list):
    ...
    if item_to_add and not any(
        ...
        gr.Info(f"'{item_to_add.get('title')}' 항목을 내 코스에 추가했습니다!") # 알림 추가
    elif not item_to_add:
        gr.Warning("추가할 항목이 없습니다. 먼저 항목을 선택해주세요.") # 경고 추가
    else:
        gr.Info(f"'{item_to_add.get('title')}' 항목은 이미 코스에 있습니다.") # 알림 추가
    ...
    if not current_course:
        ...
        for i, item in enumerate(current_course):
            ...
            exclude_cols = [
                ...
                "lclsSystm1", "lclsSystm2", "lclsSystm3", "mlevel", "cpyrhtDivCd",
                "areacode", "cat1", "cat2", "cat3", "createdtime", "mapx", "mapy",
                "modifiedtime", "sigungucode", "distance", "distance_score",
                "quarterly_trend_score", "yearly_trend_score",
                "sentiment_score", "ranking_score", "trend_reason",
                "sentiment_reason", "sub_points",
            ]
